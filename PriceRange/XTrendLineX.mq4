...
extern int TrendLineStyle = STYLE_DOT;
extern int TrendLineWidth = 1;
extern color UpperTrendLineColour = clrLimeGreen;
extern color LowerTrendLineColour = clrRed;
...
int ChartPrecision =
    5; // Number of decimal places in displayed tick chart prices
int TickChartPeriod = 1; // Period of the original tick chart data in minutes
...
int start() {
  M2O =
      PeriodSeconds(Period()) / PeriodSeconds(Period()); // Calculate multiplier
  ...
  for (int i = BarNumber; i > 0; i--) {
    ...
    if (LastPrice[i] == 0)
      LastPrice[i] = iClose(NULL, TickChartPeriod, 0);
    else
      LastPrice[i] = iClose(NULL, TickChartPeriod, 1);

    // Update offline chart data
    if (LastPrice[i] > ChartHigh[i])
      ChartHigh[i] = LastPrice[i];
    if (LastPrice[i] < ChartLow[i])
      ChartLow[i] = LastPrice[i];
    ChartClose[i] = LastPrice[i];

    RefreshRates(); // Refresh market data
    LastPrice[i] =
        NormalizeDouble(LastPrice[i], ChartPrecision); // Round tick chart price
  ...
  if (TrendLine) {
    ...
    for (int j = ObjectsTotal() - 1; j >= 0; j--) {
      ...
      if (StringSubstr(objName, 0, 4) == "HHL_")
    ...
    int stepSize =
        (int)MathPow(10, ChartPrecision); // Step size for trendline drawing
    int startBar =
        (int)(numBars - MathCeil((double)numBars / stepSize * stepSize));
